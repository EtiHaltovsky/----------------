...
//         const cost_matrix = []; // מטריצת העלות
...
//         // מילוי מטריצת העלות
//         for (let i = 0; i < Munkres.n; i++) {
//           cost_matrix[i] = [];
//           for (let j = 0; j < Munkres.n; j++) {
//             if (j >= prefName.length) {
//                 // volunteers.volunteerId
//               let volunteerId = volunteersList[i].volunteer;
//               console.log("volunteerId",volunteerId)
//               let foundVolunteer = wayA.find(item => item.volunteer === volunteerId);
//               if (foundVolunteer) {
//                 cost_matrix[i][j] = foundVolunteer.distance;
//               } else {
...
make_cost_matrix = async function (wayA) {
    ...
    try {
        ...
        for (let i = 1, j = 0; i < Munkres.n; i++) {


            cost_matrix[i][j] = preferences[i - 1].volunteerId;
            console.log("cost_matrix[i][j]", cost_matrix[i][j])
            // console.log("volunteerId",volunteerId)
            // for (const w of wayA) {
            //     console.log("w ", w)
            //     // console.log(w.volunteer + "==" + cost_matrix[i][j])
            //     // console.log("cost_matrix[i][j]", cost_matrix[i][j])
            //     if (w.volunteer == cost_matrix[i][j])
            //         console.log("nice!!!")
            //     else if (w.volunteer == cost_matrix[i][j + 1])
            //         console.log("nice!!!")
            // }
            // const way = wayA &&  wayA.find(obj => obj.volunteer.toString() == cost_matrix[i][j].toString())
            // console.log("way ", way)
        ...
        const prefName = ['selectedLanguage', 'firstAidKnowledge', 'selectedCar', 'way A']
        console.log("prefName", prefName)
        // for (let j = 1, i = 0; j < Munkres.n + 1 && prefName.length<=j; j++) {
...
//                 // console.log("volunteersListifff", volunteersList)
//           }  else{
//                 volunteersList = await volunteers.find({ fieldOfVolunteering: kindOfReq, disabledVehicle: carCheck });
// // console.log("volunteersListelseeee", volunteersList)
//        } }
...
// [2, 2, 3, 2], [1, 4, 2, 3], [1, 3, 3, 3], [1, 3, 3, 4]
// [2, 2, 4, 2], [2, 1, 4, 2], [5, 2, 1, 1], [1, 3, 3, 2]
// [1, 2, 4, 2], [1, 4, 4, 4], [2, 3, 2, 2], [2, 4, 3, 4]
// [4, 2, 3, 3], [3, 3, 3, 4], [2, 2, 2, 2], [1, 2, 3, 2]
// [1, 1, 3, 3], [1, 1, 3, 1], [1, 1, 3, 2], [4, 1, 3, 2] 
...
//     async function __make_matrix(wayA) {
//     console.log("wayyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyy",wayA)
